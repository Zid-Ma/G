...
public:
	...
	//获取成就状态，以及解锁时的解锁时间。
	//如果返回值为真，但解锁时间为零，则表示它在Steam之前已解锁
	//开始跟踪成就解锁时间（2009年12月）。自1970年1月1日以来，时间为秒。
	...
	//将当前数据存储在服务器上，设置后将收到回调
//每取得一项新成就，就有一次回访
//
//如果回调的结果是k_EResultInvalidParam，则一个或多个stats
//上传的内容被拒绝，或者是因为它们违反了限制
//或者已经过时了。在这种情况下，服务器会发回更新的值。
//应该重新迭代统计数据以保持同步。
