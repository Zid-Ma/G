// G.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//

#include "header/initialization.h"

void Loop();

int main()
{
    system("color a");
    Print::Line("Hello Gamer!");
    Initialization();
    Open_framebuffers();
    Open_shadow();
    Open_ssao();
    Loop();
}

void Loop() 
{
    Scene s;
    scene = &s;
    FixedUpdateThread = thread(FixedUpdate);
    FixedUpdateThread.detach();
    WindowOnLoad = true;
    GetTime = glfwGetTime;
    while (!glfwWindowShouldClose(Window))
    {
        Load_loop();
        Load_matrix_(&Projection_Mat4, &View_Mat4, 'p');
        Open_shadow_firstrender_begin();
        scene->Draw_Shadow();

        Open_shadow_firstrender_end();
        Open_shadow_secondrender();
        Open_ssao_firstrender();
        Open_ssao_secondrender();
        Open_ssao_threadrender();
        //Open_ssao_fourthrender();
        Open_framebuffers_firstrender();
        scene->Draw();
        Render_TextQuad();
        Open_framebuffers_secondrender();
        scene->Draw_UI();

        //Render_TextQuad();
        End_loop();
    }
    Delete_loop();
    Delete_TextQuad();
    //End_SteamAPI();
}

//销毁VBO和贴图对象
void Destory(GLuint vbo, GLuint texture)
{
    glDeleteBuffers(1, &vbo);
    glDeleteTextures(1, &texture);
}

// 运行程序: Ctrl + F5 或调试 >“开始执行(不调试)”菜单
// 调试程序: F5 或调试 >“开始调试”菜单

// 入门使用技巧: 
//   1. 使用解决方案资源管理器窗口添加/管理文件
//   2. 使用团队资源管理器窗口连接到源代码管理
//   3. 使用输出窗口查看生成输出和其他消息
//   4. 使用错误列表窗口查看错误
//   5. 转到“项目”>“添加新项”以创建新的代码文件，或转到“项目”>“添加现有项”以将现有代码文件添加到项目
//   6. 将来，若要再次打开此项目，请转到“文件”>“打开”>“项目”并选择 .sln 文件
